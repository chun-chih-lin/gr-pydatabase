      ...
      d_freq_offset_from_synclong(0.0)
{
	...
	set_msg_handler(pmt::mp("freq"), [this](const pmt::pmt_t& msg) { set_freq_by_db(msg); });
	//
...
// Add for DRL
void frame_equalizer_impl::set_freq_by_db(const pmt::pmt_t& msg) {
	size_t msg_len;
	const char* msdu;
	std::string str;
	...
	if (pmt::is_symbol(msg)) {
		...
		msg_len = str.length();
		msdu = str.data();
		set_frequency(atof(msdu));
	} else if (pmt::is_pair(msg)) {
		std::cout << "msg is pair" << std::endl;
	} else {
		std::cout << "msg is unknown type" << std::endl;
	}
}
//
